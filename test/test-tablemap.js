...
const {table, tr, td, p} = require("./build")
const {TableMap} = require("../src/tablemap")

function c(colspan, rowspan) {
...
describe("TableMap", () => {
  it("finds the right shape for a simple table", () => {
    ist(TableMap.get(table(tr(c11, c11, c11), tr(c11, c11, c11), tr(c11, c11, c11), tr(c11, c11, c11))).map.join(", "),
        "1, 6, 11, 18, 23, 28, 35, 40, 45, 52, 57, 62")
  })

  it("finds the right shape for colspans", () => {
    ist(TableMap.get(table(tr(c11, c(2, 1)), tr(c(2, 1), c11), tr(c11, c11, c11))).map.join(", "),
        "1, 6, 6, 13, 13, 18, 25, 30, 35")
  })

  it("finds the right shape for rowspans", () => {
    ist(TableMap.get(table(tr(c(1, 2), c11, c(1, 2)), tr(c11))).map.join(", "),
        "1, 6, 11, 1, 18, 11")
  })

  it("finds the right shape for deep rowspans", () => {
    ist(TableMap.get(table(tr(c(1, 4), c(2, 1)), tr(c(1, 2), c(1, 2)), tr())).map.join(", "),
        "1, 6, 6, 1, 13, 18, 1, 13, 18")
  })

  it("finds the right shape for larger rectangles", () => {
    ist(TableMap.get(table(tr(c11, c(4, 4)), tr(c11), tr(c11), tr(c11))).map.join(", "),
        "1, 6, 6, 6, 6, 13, 6, 6, 6, 6, 20, 6, 6, 6, 6, 27, 6, 6, 6, 6")
  })

  let map = TableMap.get(table(tr(c(2, 3), c11, c(1, 2)), tr(c11), tr(c(2, 1))))
  //  1  1  6 11
  //  1  1 18 11
  //  1  1 25 25
