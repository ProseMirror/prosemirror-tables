    ...
    state: {
      init(_, state) {
        this.spec.props.nodeViews[tableNodeTypes(state.schema).table.name] = (node, view) =>
            new View(node, cellMinWidth, maxTableWidth, view);
    ...
    props: {
      ...
      handleDOMEvents: {
        ...
        mousedown(view, event) {
          handleMouseDown(view, event, cellMinWidth, maxTableWidth);
        },
      },
...
function handleMouseDown(view, event, cellMinWidth, maxTableWidth) {
  let pluginState = key.getState(view.state);
  if (pluginState.activeHandle == -1 || pluginState.dragging) return false;

  let cell = view.state.doc.nodeAt(pluginState.activeHandle);
  let width = currentColWidth(view, pluginState.activeHandle, cell.attrs);
  let finalWidth = currentColWidth(view, pluginState.activeHandle, cell.attrs);
  ...
  function move(event) {
    ...
    finalWidth = displayColumnWidth(view, pluginState.activeHandle, dragged, cellMinWidth, maxTableWidth);
  }
...
function displayColumnWidth(view, cell, width, cellMinWidth, maxTableWidth) {
  let $cell = view.state.doc.resolve(cell);
  let table = $cell.node(-1),
      start = $cell.start(-1);
  ...
  return updateColumns(table, dom.firstChild, dom, cellMinWidth, maxTableWidth, col, width);
}
